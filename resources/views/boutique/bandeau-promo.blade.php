...
background: radial-gradient(circle, rgba(252,235,145,1) 0%, rgba(255,225,64,1) 100%); color: black; z-index: 1;">
        ...
        <h4>Du <span class="fs-3 fw-bold">{{ date('d-m-Y', strtotime($promo->date_debut)) }}</span> au
            <span class="fs-3 fw-bold">{{ date('d-m-Y', strtotime($promo->date_fin)) }}</span>
        </h4>
        <h4>PROMOTION SPÉCIALE " <span class="text-uppercase fs-3 fw-bold">{{ $promo->nom }}</span> "</h4>
        <h4><span class="fw-bold fs-3">-{{ $promo->reduction }}%</span> sur une sélection de produits !</h4>
